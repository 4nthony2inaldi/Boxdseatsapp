...
- **MLB spring 2023 gap** (data fix): the historical preseason backfill (→2022) and current-era seed (2024+) left a seam — spring training 2023 was missing league-wide (only MLB; other leagues' 2023 preseason was fine). Re-seeded 459 games via seed-real-data.mjs --from=2023-02-20 --to=2023-04-02 and transplanted. Note: gen-transplant-v2 re-emits all PGA rows as defensive uuid-updates; when staging hasn't corrected anything, delete the event-upd batches and apply only inserts/venue/athlete files.
- **Era-split venue merges** (data fix, prod + staging): ESPN assigns a NEW venue id when a stadium gets a sponsor rename, splitting one building across rows (Dunedin was a triplet: Florida Auto Exchange Stadium 2002-18 / Dunedin Stadium 2019-20 / TD Ballpark 2021+). Merged via temp merge_venue() (repoints events/event_logs/venue_visits/profiles.fav_venue_id/user_league_favorites/list_items/venue_teams/venue_aliases, era names kept in venue_name_at_time, old names become aliases, old espn ids folded into external_ids.espn_legacy; SQL in /tmp/prod/merge-venues.sql pattern). Merged: Dunedin → TD Ballpark; Lee County → Lee Health Sports Complex (Hammond Stadium); Tahoe Mt. Club → Tahoe Mountain Club. Policy: REPLACED buildings stay separate venues (Veterans→Citizens Bank etc.); RENAMED buildings merge. Audit query: same city+primary_sport venue pairs with adjacent non-overlapping event ranges (<1200-day gap) — rerun after historical ingests. Tennis "Site" sponsor-variants remain the big unmerged family (documented future cleanup).

### Session 13 addendum — systematic gap audit ("stadiums not going back far enough")
Root cause of every per-venue gap: the original backfill skipped any ESPN game without venue data ("no venue in payload", silently counted in eventsErrored). ESPN lacks venues for: nearly all 2002-05 NFL (+scattered 2006-07), 2002-04 MLS, defunct arenas (Compaq Center, Pyramid, Alamodome era), Cubs spring 2014-16 (Sloan Park pre-dating its ESPN venue id), and spring 2021 at Camelback/Roger Dean/Clover/CACTI/Hammond. Fixes (all in prod + staging):
- **seed-real-data.mjs HOME_VENUE_FALLBACKS**: curated home-team era-venue map (NFL 32 teams incl. Bears'02→Champaign, Saints'05 relocation by date; MLS 2002-04 grounds; NBA Compaq/Pyramid/Alamodome; MLB spring parks). Re-seeded ~2,130 games. Also CONFERENCE_TEAM_RE (2000s Pro Bowls named "NFC at AFC" leak conference pseudo-teams).
- **gen-transplant-v2 espn ids are now league-scoped** — 2002-era ESPN ids are date-encoded and collide ACROSS leagues (same id = an NHL and an NFL game); the global set silently dropped ~224 inserts.
- **More era-split merges** (scripts/data/005): Spectrum Field→BayCare Ballpark (overlapping COVID-2020 rows — the audit needs overlap tolerance ~400d, not strict adjacency), RFK ×3 spellings, Pizza Hut Park→Toyota Stadium, Marlins Park→loanDepot Park, Miller Park→American Family Field, Monster Park→Candlestick Park. Cubs 2014 spring homes repointed Hohokam→Sloan ('Cubs Park'). **Montreal Expos 2002-04 home games (199) were on "Olympic Stadium Berlin"** — row fixed to Olympic Stadium, Montreal QC.
- **88 dual-scheme dupes deleted** (scripts/data/006): same game under legacy 9-digit AND modern 400-prefix ESPN ids; same-scheme same-score pairs are real doubleheaders, kept.
- New era venues: Sun Devil Stadium, Memorial Stadium (Champaign), Compaq Center, Pyramid Arena, Tiger Stadium (Baton Rouge), Cardinal Stadium (Naperville).
